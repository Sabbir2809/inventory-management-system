...
import { useReturnReportQuery } from "../../redux/features/report/reportApi";

const ReturnReport = () => {
  const [expenseData, setReturnData] = useState({
  ...
  const { data: returnReportData } = useReturnReportQuery(expenseData);
  ...
  return (
    <Col>
      <Card bordered={true} style={{ marginBottom: 8 }}>
        <Typography.Title level={3}>Generate Return Report</Typography.Title>
        <Col span={24} lg={12}>
      ...
      {/* download format */}
      {returnReportData?.data?.rows.length > 0 && (
        <Card bordered={true} style={{ marginBottom: 8 }}>
          <Typography.Title level={3}>Total: {returnReportData?.data?.total} Tk</Typography.Title>
          <Col span={24} lg={12}>
            <Button
              style={{ marginRight: "10px" }}
              onClick={() => onExport("csv", "Return-Report", returnReportData?.data?.rows)}>
              Download CSV
            </Button>
            <Button onClick={() => onExport("xls", "Return-Report", returnReportData?.data?.rows)}>
              Download XLS
            </Button>
          </Col>
        </Card>
